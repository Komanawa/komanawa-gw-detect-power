...
#. `Understanding_detection_power <./Understanding_detection_power.ipynb>`_: A brief introduction to what detection power is, why it is important, and how it can be used to design monitoring programs.  This notebook also discusses the difference between slope detection power and counterfactual detection power and when to use each.
#. `Let's talk noise <./lets_talk_noise.ipynb>`_: A brief introduction to the concept of measurement noise and unexplained variance in environmental time series and how it can be estimated. The focus of this notebook is on understanding the concept of noise and how it can be estimated from data, rather than using the tools provided in this package.
#. `Generating_true_ts_data <./generating_true_ts_data.ipynb>`_: A brief introduction to generating noise free time series data accounting for the influence of groundwater lag.  This example discusses creating your own true time series data as well as using the inbuilt functions to create true time series data for a range of scenarios.
#. `Slope_detection_power <./slope_detection_power.ipynb>`_: This example discusses what slope detection power is and the different statistical tests that can be used to detect a slope in time series data. The examples showcase the implemented statistical tests for the package and provide visual examples of what each test is doing.
#. `Counterfactual_detection_power <./counterfactual_detection_power.ipynb>`_: This example provides details on counterfactual detection power. It provides visual examples of what counterfactual detection power is and how it differs from slope detection power. It also provides examples of how to use the package to calculate counterfactual detection power for a range of scenarios.
#. `Whakauru_Stream_worked_example <./Whakauru_Stream_worked_example.ipynb>`_: A full worked example of evaluating the detection power of the Whakauru Stream. This stream is primarily fed by groundwater and has seen a significant increase in nitrate concentrations. This example shows how to use the package to assess the detection power of the monitoring program for a scenario that reduces nitrate concentrations by approximately 50% over 20 years.
#. `Selwyn_well_I36_0477_worked_example <./Selwyn_well_I36_0477_worked_example.ipynb>`_: A full worked example of evaluating the detection power of the Selwyn well I36/0477. This well has significant lag and therefore implementing a 15% reduction in nitrate source concentrations will never reduce the nitrate concentrations in the well (they will plateau lower than if the reductions were not implemented, but they will never reduce). This example demonstrates a real world example of a scenario where slope detection power is not appropriate and counterfactual detection power should be used instead.
#. `Counterfactual_from_models <./counterfactual_from_models.ipynb>`_: A full worked example of evaluating the detection power for a site for synthetic modelled scenario output.  This example serves to demonstrate how more complex model outputs can still be used within the detection power calculator framework.  This example uses synthetic data, but the same process can be applied to real model outputs.
#. `Increasing_run_efficiency <./increasing_run_efficency.ipynb>`_: A worked example of how to increase the run efficiency of the detection power calculator. For very large runs and complex methods (e.g., Multipart Mann-Kendall tests) the run time can be significant.  This examples provides some tips and tricks for increasing the run efficiency.
