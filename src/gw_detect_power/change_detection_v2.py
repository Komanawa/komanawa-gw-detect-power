...
class BaseDetectionCalculator:
    ...
    def resample_timeseries(self):  # todo helper function to resample the time series
        raise NotImplementedError
    ...
    @staticmethod
    def _get_key_info(key):  # todo need to look into whether or not it can actually be None a lot cannot be
        ...
        data = dict(
            ...
            max_conc_lim_vals  # todo make sure this happens
        min_conc_lim_vals  # todo make sure this happens
        mrt_vals
...
class DetectionPowerCalculator:
    def __init__(self, *args, **kwargs):
        raise NotImplementedError  # todo as a place holder to refactor tests


class DetectionPowerSlope(BaseDetectionCalculator):  # # todo pass own concentration
    ...
    def _multiprocess_checks(self, outpath, id_vals, **kwargs):
        ...
        if 'mrt_model_vals' in kwargs:
            ...
            if 'max_conc_lim_vals' in kwargs:  # todo ensure that this is passed
            ...
            if ('true_conc_ts_vals' in kwargs):
    ...
    def mulitprocess_power_calcs(self,
        ...
        """
        # todo run _multiprocess_checks
        raise NotImplementedError  # todo


class AutoDetectionPowerSlope(DetectionPowerSlope):  # todo don't pass own concentration
    implemented_mrt_models = (
        'piston_flow',
    ...
    def power_calc(self,  # todo check options, most nones are gone
                   ...
                   max_conc_lim: {float, None} = None,
                   min_conc_lim: {float, None} = None,
                   mrt: {float, None} = None,
        ...
        :param seed: int or None for random seed
        :param testnitter: None (usually) or a different nitter then self.niter for testing run times
        :param kwargs: kwargs passed to the output series
                      (e.g. region='temp' will yield a 'region' index with a value of 'temp')
        :return: pd.Seris with the power calc results note power is percent 0-100
                Possible other dataframes if self.return_true_conc is True or self.return_noisy_conc_itters > 0
                in which case a dictionary will be returned:
                    [
                    'power': power_df, # always
